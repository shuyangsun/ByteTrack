...
for ((i = start_position; i < start_position + num_files_to_process && i < total_files; i++)); do
    ...
    python tools/demo_track.py video -f exps/example/mot/yolox_x_ablation.py -c ${PATH_TO_BTRACK_MODEL} --device ${device} --fps 15 --fp16 --batch_size 64 --fuse --save_result --path ${file_name}
done
